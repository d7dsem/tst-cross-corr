...
if __name__ == "__main__":
    ...
    try:
        ...
        signal, sync = gen_sign(SYNC_LN=SYNC_LN,M=M,SUPER_FRAME_LN=SUPER_FRAME_LN,SF_COUNT=SF_COUNT,GUARD=GUARD,SPS=SPS,SIGN_STD=SIGN_STD,seed_sync=SEED_SYNC)
        ...
        if 1: 
            print(f"{INFO} Signal power: {CYAN}{signal_power:.2f}{RESET}  SING_STD: {CYAN}{SIGN_STD}{RESET}")
        ...
        print(f"{INFO} Target SNR: {CYAN}{TARGET_SNR_DB:.2f}{RESET} dB, actual {CYAN}{snr_actual:.2f}{RESET} dB  noise std: {CYAN}{noise_std:.2f}{RESET}")
        print(f"       sign_std: {CYAN}{SIGN_STD}{RESET}    noise std: {CYAN}{noise_std:.2f}{RESET}")
        ...
        peaks, vals, corr = detect_sync_positions(signal_in=signal_noisy, sync=sync, min_distance=SPS*(SYNC_LN+2*M))
        plot_cross_corr(peaks, vals, corr, title=f"Cross-correlation with noisy [snr={TARGET_SNR_DB} dB] signal", block=False)
        
        print(f"{INFO} Corr length: {len(corr)}, peaks found: {len(peaks)}")
